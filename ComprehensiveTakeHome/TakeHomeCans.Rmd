...
```{r}
medCONT <- median(CONT$TotalDelay, na.rm = TRUE)
medEXP <- median(EXPRESS$TotalDelay, na.rm = TRUE)
medCONT
```
...
set.seed(123)
for (i in 1:R) {
  index <- sample(length(Q4B$TotalDelay), replace = FALSE)
  TS[i] <- median(Q4B$TotalDelay[index]) - median(Q4B$TotalDelay[-index])
}
pvalue <- (sum(TS >= 7) + 1)/(R+1)
pvalue
```
