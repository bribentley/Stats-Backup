...
I started this competition by just focusing on getting a good understanding of the data set. The EDA is detailed and many visualizations are included. This version also includes modeling. From the glmnet family of models, lasso performs best with regards to both the cross validation scores and public scores. The second models that I composed is an XGBoost model. Taking the average of those 2 models gives me the best public score at the moment.
...
The train data set consist of character and integer variables. Most of the character variables are actually (ordinal) factors, but I chose to read them into R as character strings as most of them require cleaning and/or feature engineering first. In total, there are 81 columns/variables, of which the last one is the response variable (SalePrice). Below, I am displaying only a glimpse of the variables. All of them are discussed in more detail throughout the document.
...
The character variables need some work before I can use them. To get a feel for the data set, I decided to first see which numeric variables have a high correlation with the SalePrice.
...
Please be aware that this model is a draft model. I have done done full hyper parameter tuning yet. In this version, I have only decreased the eta and left all other parameters at their defaults for the time being. I will do full hyper parameter tuning with the `mlr` package in the next version. 
