...
2.  How many patients should you include in your sample? 
    a.  Use a *z-test* to answer the question. 317 *shiny app*
    b.  Use a *t-test* to answer the question. 43
    
For the z distribution
```{r}
z <- qnorm(.01, 300, 50/sqrt(40))
z
POWER <- pnorm(z, 275, 50/sqrt(40))
POWER
```
...
I do **???** think this assumption is reasonable 
...
Based off of the lead physician's belief that the standard deviation will be between 40 and 70 mg/dL, we suggest a sample size of at least 82 participants.
    ...
    In this situation, a type I error would be when we reject the mean of 300 mg/dL when it is actually true.     A type II error would be when we fail to reject the mean of 300 mg/dL when it is actually false.
...
beta <- .2
alpha <- 0.22
power.t.test(n = NULL, delta = 25, sd = 70, sig.level = 0.22,
              power = 1 - beta, type = "one.sample", alternative = "one.sided")$n
```

A sample size of 21 is needed to ensure the type II error is no greater than the type I error at a power of 80%.
