...
Populations are normal - qqplots, density plots, ggplot2, side by side boxplots or ftest (vartest) to test variances

```{r}
#equivalent to qqnorm plot
library(ggplot2)
...
Extrememly skewed right distributions and the variances don't look similar either.
...
Based on this F test (Null = ratio of variances is 1), since the p-value is so small and the conf level does not include 1 we can therefore reject the null hypothesis. So the variances are not the same.

So should not use a two sample t test. So how do we test it?
...
test_stat <- t.test(estimate ~ unit, data = roomwidth, var.equal = FALSE)$stat
test_stat
DF <- t.test(estimate ~ unit, data = roomwidth, var.equal = FALSE)$parameter
DF
#Upper and Lower rejection regions
LRR <- qt(0.025, DF)
URR <- qt(0.975, DF)
c(LRR, URR)
```

The observed standardized test statistic is $test_{stat}=`r test_stat`$, and the lower and upper rejection regions for a two sided alternative are `r LRR`, and `r URR`, respectively.

4. Statistical Conclusion

a.  Reject the null hypothesis since $test_{stat}=`r test_stat` < `r LRR`$.
b.  Reject the null hypothesis since the *p*-value `r t.test(estimate ~ unit, data = roomwidth)$p.value` is less than 0.05.

```{r}
pvalue <- pt(test_stat, DF)*2
pvalue
```
...
* Wilcox Test - Tests for the medians
...
```{r}
# Code to compute power here (cv = critical value)
...
```{r}
# Compute ncp (non-centrality paramter), cv (critical value), and power here
...
```{r}
# Estimate of nu (greek letter for degrees of freedom), cv, and power here 
...
ncp1 <- 5/(sqrt(12.56^2/69 + 23.4^2/44))
ncp1
# Estimate of nu 
